...
public class RadialTimePickerDialog extends DialogFragment implements
	...
	public static RadialTimePickerDialog newInstance(
			OnTimeSetListener callback, int hourOfDay, int minute,
			boolean is24HourMode, boolean isSwitchOn) {
		RadialTimePickerDialog ret = new RadialTimePickerDialog();
		ret.initialize(callback, hourOfDay, minute, is24HourMode, isSwitchOn);
		return ret;
	}

	public void initialize(OnTimeSetListener callback, int hourOfDay,
			int minute, boolean is24HourMode, boolean isSwitchOn) {
		mCallback = callback;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mSwitch = (Switch) view.findViewById(R.id.repeat_switch);
		...
		mTimePicker.initialize(getActivity(), mHapticFeedbackController,
				mInitialHourOfDay, mInitialMinute, mIs24HourMode);
		...
		view.findViewById(R.id.line).setBackgroundColor(
				mThemeDark ? darkLine : line);
		mDoneButton
				.setTextColor(mThemeDark ? darkDoneTextColor : doneTextColor);
		mTimePicker.setBackgroundColor(mThemeDark ? lightGray
				: circleBackground);
		mDoneButton.setBackgroundResource(mThemeDark ? darkDoneBackground
				: doneBackground);

		setEnabled(isSwitchOn);
		return view;
	}

	public void setEnabled(boolean isEnable) {
		mHourView.setEnabled(isEnable);
		mHourSpaceView.setEnabled(isEnable);
		mMinuteSpaceView.setEnabled(isEnable);
		mDoneButton.setEnabled(isEnable);
		mMinuteView.setEnabled(isEnable);
		mAmPmTextView.setEnabled(isEnable);
		mTimePicker.setEnabled(isEnable);
	}
